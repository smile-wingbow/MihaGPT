...
class CollectLinks(Action):
    ...
    async def _search_and_rank_urls(self, topic: str, query: str, num_results: int = 1) -> list[str]:
        ...
        """
        max_results = max(num_results * 2, 3)
        results = await self.search_engine.run(query, max_results=3, as_string=False)
